...
export default function Otp() {
    ...
    async function confirmOtp(values) {
        try {
            const response = await apiClient.post("Auth/confirm-email-otp", values)
    ...
    async function resendOtp() {
        try {
            const response = await apiClient.post("Auth/resend-confirmation-otp", {
                // email: "nouran3ly@gmail.com"
                email: email
            })
            console.log(response);
        } catch (error) {
            console.log(error.response.data);
            setuserMsg(error.response.data)
        }
    }
    ...
    return (
        <div className="flex justify-center items-center min-h-[calc(100vh-70px)] md:my-0 bg-[#152a4c]">
            <div className="bg-white rounded-2xl px-5 md:px-10 py-8 md:py-10">
                <h1 className='font-bold text-xl text-center'>OTP Verification</h1>
                ...
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <h2 className='text-center'>Haven’t got the code yet? <button className='underline text-orange' onClick={() => { resendOtp() }}>Resend code</button></h2>
